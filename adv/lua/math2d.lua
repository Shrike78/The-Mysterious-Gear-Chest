...
function M.doIntersect(p1,q1,p2,q2)
	...
	-- special cases
	o1 = M.orientation(p1, q1, p2)
	o2 = M.orientation(p1, q1, q2)
	o3 = M.orientation(p2, q2, p1)
	o4 = M.orientation(p2, q2, q1)
	...
	if (o1 == 0 and M.onSegment(p1, p2, q1)) then return true end
	-- p1, q1 and p2 are colinear and q2 lies on segment p1q1
	if (o2 == 0 and M.onSegment(p1, q2, q1)) then return true end
	--  p2, q2 and p1 are colinear and p1 lies on segment p2q2
	if (o3 == 0 and M.onSegment(p2, p1, q2)) then return true end
	-- p2, q2 and q1 are colinear and q1 lies on segment p2q2
	if (o4 == 0 and M.onSegment(p2, q1, q2)) then return true end
...
function M.isInside(listPoints,p,h)
	...
	for i=1,#listPoints do
		...
		if (M.doIntersect(listPoints[i], listPoints[ip], p, extreme)) then
			-- If the point 'p' is colinear with line segment 'i-ip',
			-- then check if it lies on segment. If it lies, return true,
			-- otherwise false
			if (M.orientation(listPoints[i], p, listPoints[ip]) == 0) then
				return M.onSegment(listPoints[i], p, listPoints[ip])
			end
			count = count+1
		end
	end
